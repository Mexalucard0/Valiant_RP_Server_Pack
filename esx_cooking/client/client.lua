...
Citizen.CreateThread(function()
	while true do
		...
		--Taco Shop
		if GetDistanceBetweenCoords(coords, Config.Zones.FoodOrder3.x, Config.Zones.FoodOrder3.y, Config.Zones.FoodOrder3.z, true) < Config.DrawDistance and hasProperty2 then
            DrawText3Ds(Config.Zones.FoodOrder3.x, Config.Zones.FoodOrder3.y, Config.Zones.FoodOrder3.z + 0., "Press [~g~E~s~] to open the order menu", 0.4)
		end
		       
		if GetDistanceBetweenCoords(coords, Config.Zones.FoodOrder3.x, Config.Zones.FoodOrder3.y, Config.Zones.FoodOrder3.z, true) < 1 and hasProperty2 then
			if IsControlJustReleased(0, Keys['E']) then
				OpenMenu(true)
			end
		end

		if GetDistanceBetweenCoords(coords, Config.Zones.KitchenMenu3.x, Config.Zones.KitchenMenu3.y, Config.Zones.KitchenMenu3.z, true) < Config.DrawDistance and hasProperty2 then
            DrawText3Ds(Config.Zones.KitchenMenu3.x, Config.Zones.KitchenMenu3.y, Config.Zones.KitchenMenu3.z + 0., "Press [~g~E~s~] to open the kitchen menu", 0.4)
		end

		if GetDistanceBetweenCoords(coords, Config.Zones.KitchenMenu3.x, Config.Zones.KitchenMenu3.y, Config.Zones.KitchenMenu3.z, true) < 1 and hasProperty2 then
			if IsControlJustReleased(0, Keys['E']) then
				OpenMenu2(true)
			end
		end		
...
RegisterCommand("accept", function()
	...
	--Taco
	if GetDistanceBetweenCoords(coords, Config.Zones.RecieveFood3.x, Config.Zones.RecieveFood3.y, Config.Zones.RecieveFood3.z, true) < 2 then
		for k, v in pairs(orders) do
			TriggerServerEvent("esx_cooking:giveItem", v)
		end
...
--Taco
RegisterNUICallback("buytacomeat", function()
	ESX.TriggerServerCallback("checkMoney", function(cb)
		if cb then
			ESX.ShowNotification("~g~You have ordered ~y~1x ~g~taco meat~s~." .. " ~y~Please wait for your order~s~.")
			Wait(60000)
			ESX.ShowNotification("~g~Your order was been delivered on your door.~s~")
            table.insert(orders, "tacomeat")
		else
			ESX.ShowNotification("~r~You don't have enough money!~s~")
		end
	end)
...
RegisterNUICallback("buyspices", function()
	ESX.TriggerServerCallback("checkMoney3", function(cb)
		if cb then
			ESX.ShowNotification("~g~You have ordered ~y~1x ~g~spices~s~." .. " ~y~Please wait for your order~s~.")
			Wait(60000)
			ESX.ShowNotification("~g~Your order was been delivered on your door.~s~")
            table.insert(orders, "spices")
		else
			ESX.ShowNotification("~r~You don't have enough money!~s~")
		end
	end)
...
RegisterNUICallback("maketaco", function()
	OpenMenu2(false)
	ESX.TriggerServerCallback("esx_cooking:checkItem1", function(cb1)
		if cb1 then
			ESX.ShowNotification("~y~You're preparing your taco~s~")
			TaskStartScenarioInPlace(PlayerPedId(), "PROP_HUMAN_BBQ", 0, false)
			Wait(2*10000)
			ClearPedTasksImmediately(PlayerPedId())
			TriggerServerEvent("esx_cooking:giveitem4")
		else
			ESX.ShowNotification("~r~You don't have enough ingredients to perpare this food~s~")
		end
	end, "tacomeat", "spices")
