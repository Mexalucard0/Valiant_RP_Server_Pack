...
ESX.RegisterUsableItem('repairkit', function(source)
	...
	if Config.AllowMecano then
		TriggerClientEvent('esx_repairkit:onUse1', _source)
	else
...
ESX.RegisterUsableItem('advancedrepairkit', function(source)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
AddEventHandler('esx_repairkit:removeKit', function()
	...
	if not Config.InfiniteRepairs then
		xPlayer.removeInventoryItem('advancedrepairkit', 1)
		TriggerClientEvent('esx:showNotification', _source, _U('used_kit'))
	end
end)

RegisterNetEvent('esx_repairkit:removeKit1')
AddEventHandler('esx_repairkit:removeKit1', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
