...
ESX.RegisterServerCallback('valiant_headlights:check', function(source, cb, plate)
...
RegisterServerEvent('valiant_headlights:set')
AddEventHandler('valiant_headlights:set', function(plate, color)
    MySQL.Sync.execute("UPDATE owned_vehicles SET color =@color WHERE plate=@plate",{['@color'] = color, ['@plate'] = plate})
end)

RegisterServerEvent('valiant_headlights:install')
AddEventHandler('valiant_headlights:install', function(plate)
    local _src = source
