...
function OpenMechanicActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mechanic_actions', {
		...
		if data.current.value == 'vehicle_list' then
			if Config.EnableSocietyOwnedVehicles then
				...
				local elements = {
					...
					{label = _U('Track_Car'), value = 'maliboohoo'},
					{label = _U('Contender'), value = 'contender'},
					{label = _U('Bison'), value = 'Bison'},
					{label = _U('Trailer'), value = 'trailersmall'}										
				}
		...
		elseif data.current.value == 'boss_actions' then
			TriggerEvent('esx_society:openBossMenu', 'mechanicc', function(data, menu)
				menu.close()
			end)
		end
	end, function(data, menu)
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
				...
				if amount == nil or amount < 0 then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						menu.close()
						TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_mechanicc', _U('mechanicc'), amount)
					end
				end
			end, function(data, menu)
				menu.close()
...
AddEventHandler('esx_mechanicjob:hasEnteredEntityZone', function(entity)
	...
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanicc' and not IsPedInAnyVehicle(playerPed, false) then
...
--	local blip = AddBlipForCoord(Config.Zones.MechanicActions.Pos.x, Config.Zones.MechanicActions.Pos.y, Config.Zones.MechanicActions.Pos.z)
	...
	AddTextComponentSubstringPlayerName(_U('mechanicc'))
...
Citizen.CreateThread(function()
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanicc' then
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanicc' then
				...
				elseif CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage', 'mechanicc', vehicleProps)
						...
						then
							TriggerServerEvent('esx_service:disableService', 'mechanicc')
						end
		...
		if IsControlJustReleased(0, 167) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanicc' then
			OpenMobileMechanicActionsMenu()
		end

		if IsControlJustReleased(0, 178) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanicc' then
			if NPCOnJob then
