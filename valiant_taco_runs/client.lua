...
local Vtacos = { x = 14.0, y = -1598.0, z = 28.36} --Start service
local Vtacosfin = { x = 5.24, y = -1605.28, z = 28.4} --Finish service
local spawnr1 = { x = 17.36, y = -1611.96, z = 29.28 } --Vehicle spawn
...
local blips = {
  {title="Vtacos", colour=51, id=208, x = 11.08, y = -1605.84, z = 29.4}, --Map Blip
}

local coefflouze = 0.1 --Job distance multiplier
...
Citizen.CreateThread(function() --adding the point of the Vtacos on the map
...
Citizen.CreateThread(function() --launch + delivery from the green marker
  while true do
    ...
    if isInJobPizz == false then
      ...
      DrawMarker(1,Vtacos.x,Vtacos.y,Vtacos.z, 0, 0, 0, 0, 0, 0, 1.5001, 1.5001, 0.6001,0,255,0, 200, 0, 0, 0, 0)

      if GetDistanceBetweenCoords(Vtacos.x, Vtacos.y, Vtacos.z, GetEntityCoords(GetPlayerPed(-1),true)) < 1.5 then
        HelpText("Press ~INPUT_CONTEXT~ to start delivery of ~r~tacos",0,1,0.5,0.8,0.6,255,255,255,255)
        ...
        if IsControlJustPressed(1,38) then
            ...
            py = livpt[livr].y
            pz = livpt[livr].z
            distance = round(GetDistanceBetweenCoords(Vtacos.x, Vtacos.y, Vtacos.z, px,py,pz))
            paie = distance * coefflouze

            spawn_r1()
            goliv(livpt,livr)
            nbtaco = math.random(1, 3)

            TriggerServerEvent("taco:itemadd", nbtaco)
        end
      end
    end
    ...
    if isToHouse == true then
      ...
      while notif == true do
        ...
        TriggerEvent("pNotify:SendNotification", {
          text = "Direction : " ..destinol.. " pour livrer la taco",
          type = "info",
          queue = "global",
          timeout = 4000,
          layout = "bottomRight"
      ...
      if GetDistanceBetweenCoords(px,py,pz, GetEntityCoords(GetPlayerPed(-1),true)) < 3 then
        HelpText("Press ~INPUT_CONTEXT~ to deliver the taco",0,1,0.5,0.8,0.6,255,255,255,255)

        if IsControlJustPressed(1,38) then

          notif2 = true
          posibilidad = math.random(1, 100)
          afaitunetacomin = true

          TriggerServerEvent("taco:itemrm")
          nbtaco = nbtaco - 1
          ...
          if (posibilidad > 70) and (posibilidad < 90) then
            ...
            TriggerServerEvent("taco:pourboire", pourboire)
    ...
    if isToVtacos == true then

      while notif2 == true do
        SetNewWaypoint(Vtacos.x,Vtacos.y)

        TriggerEvent("pNotify:SendNotification", {
          text = "Direction la Vtacos !",
          type = "info",
          queue = "global",
          timeout = 4000,
          layout = "bottomRight"
      ...
      if GetDistanceBetweenCoords(Vtacos.x,Vtacos.y,Vtacos.z, GetEntityCoords(GetPlayerPed(-1),true)) < 3 and afaitunetacomin == true then
        HelpText("Press ~ INPUT_CONTEXT ~ to collect the tacos",0,1,0.5,0.8,0.6,255,255,255,255)

        if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("r1"))  then

          if IsControlJustPressed(1,38) then

            if IsInVehicle() then

              afaitunetacomin = false
              ...
              TriggerServerEvent("taco:pourboire", paie)
...
Citizen.CreateThread(function() -- Thread de "fin de service" depuis le point rouge
  while true do
    ...
    if isInJobPizz == true then
      ...
      DrawMarker(1,Vtacosfin.x,Vtacosfin.y,Vtacosfin.z, 0, 0, 0, 0, 0, 0, 1.5001, 1.5001, 0.6001,255,0,0, 200, 0, 0, 0, 0)

      if GetDistanceBetweenCoords(Vtacosfin.x, Vtacosfin.y, Vtacosfin.z, GetEntityCoords(GetPlayerPed(-1),true)) < 1.5 then
        HelpText("Press ~ INPUT_CONTEXT ~ to stop the delivery of ~ r ~ taco",0,1,0.5,0.8,0.6,255,255,255,255)

        if IsControlJustPressed(1,38) then
          TriggerServerEvent('taco:deleteAllPizz')
          isInJobPizz = false
          livr = 0
          isToHouse = false
          isToVtacos = false
          ...
          if afaitunetacomin == true then
            ...
            TriggerServerEvent("taco:paiefinale")
...
function spawn_r1() -- Thread spawn r1

  Citizen.Wait(0)

  local myPed = GetPlayerPed(-1)
  local player = PlayerId()
  local vehicle = GetHashKey('r1')
  ...
  local spawned_car = CreateVehicle(vehicle, spawnr1.x,spawnr1.y,spawnr1.z, 431.436, - 996.786, 25.1887, true, false)

  local plate = "sctyownd"..plateJob
